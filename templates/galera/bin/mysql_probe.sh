...
function check_mysql_status {
    local status=$1
    local expect=$2
    local val=$(get_mysql_status "${status}")
    local rc
...
function check_mysql_startup {
    ...
    # a joiner node must have an established socket connection before testing further
        ...
        local connections=$(ss -tnH state established src :4567 or dst :4567 | wc -l)
    ...
    if [ -e /var/lib/mysql/mysql.sock ]; then
        ...
        local status=$(get_mysql_status wsrep_cluster_status)
        local comment=$(get_mysql_status wsrep_local_state_comment)
